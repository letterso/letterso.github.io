...
- `program` 要调试的程序名，`${workspaceFolder}`为当前工作目录，即`.vscode`同级目录
- `cwd` 调试程序的路径
- `miDebuggerPath` gdb 的路径

高级参数

- `"preLaunchTask": "build"` 生成子 task，可在里面编辑 gcc 命令等。具体可参考 [VSCode GDB 调试配置](https://blog.csdn.net/bingyu9875/article/details/94745028)
...
通过在launch中设置`launch-prefix`启动调试，参考
...
C++程序可增加`launch-prefix="xterm -e gdb -ex run --args "`

python程序可增加`launch-prefix="xterm -e python -m pdb "`
...
[ros项目调试:ROS项目使用GDB调试](https://blog.csdn.net/weixin_35695879/article/details/85254399)

# 信号处理

linux下sig类型，可以通过`kill -${sig_num} ${PID}`给对应进程发送信号。
...
- SIGABRT 异常终止条件，例如为 [std::abort()](https://c-cpp.com/cpp/utility/program/abort.html) 所起始
- SIGFPE 错误算术运算，例如除以零
- SIGPIPE PIPE破裂
...
- **静态分析**是在代码编译前发现潜在问题，覆盖所有代码路径（无论是否被执行），但可能存在误报，且对复杂运行时问题（如动态内存管理和并发）的检测能力有限。
...
| **误报率**           | 较低                                           | 较低到中等（`clang-tidy`规则多可能报告更多“建议”）           | 极低（几乎无误报）                                           | 极低（运行时发现的问题通常是真实存在的）            | 中高（基于模式匹配可能误报）               |
...
- `--std=<id>`：设置 C/C++ 标准。
- `--language=<language>`：强制 Cppcheck 将所有文件作为给定语言检查，有效值为 `c` 或 `c++`。
...
[Sanitizer](https://github.com/google/sanitizers/wiki/)是由Google发起的开源工具集，用于检测内存泄露等问题。
...
参考[sanitizers-cmake](https://github.com/arsenm/sanitizers-cmake)实现调用
...
报错找不到`libasan.so.5`库，设置`-static-libasan`，链接静态库。
...
[开发调试利器 | 内存检测工具Sanitizer](https://zhuanlan.zhihu.com/p/691182407)

[【NO.114】C++内存管理及内存问题的分析](https://github.com/0voice/cpp_backend_awsome_blog/blob/main/%E3%80%90NO.114%E3%80%91C%2B%2B%E5%86%85%E5%AD%98%E7%AE%A1%E7%90%86%E5%8F%8A%E5%86%85%E5%AD%98%E9%97%AE%E9%A2%98%E7%9A%84%E5%88%86%E6%9E%90.md)
